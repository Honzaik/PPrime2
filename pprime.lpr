...
2 : c1<c2
  ...
  var i : integer;
  begin
    for i := 1 to delka do c1.cislo[i] := not c1.cislo[i];
    for i := (delka+1) to MAX_DELKA_BIN do c1.cislo[i] := false;
    c1.delka := delka;
    flip := c1;
  end;

  function subBin(c1, c2 : cisloBin) : cisloBin;
  var jedna, mezi : cisloBin;
  var i : integer;
  begin
       jedna.delka := 0;
       vynulujBin(jedna);
       ...
       i := mezi.delka;
       while (mezi.cislo[i] <> true) and (i > 0) do i := i-1;
       if(i > 0) then mezi.cislo[i] := false;
       while (mezi.cislo[i] <> true) and (i > 0) do i := i-1;
       mezi.delka := i;
       subBin := mezi;
  end;
...
var vysl, mult, scaledDivisor, remain : cisloBin;
begin
  mult.delka := 1;
  vynulujBin(mult);
  mult.cislo[1] := true;
  ...
  repeat
        ...
        begin
          ...
          //writeln('adding');
          //vypisBin(otocBin(vysl));
          //vypisBin(otocBin(mult));
          vysl := addBin(vysl, mult);
          //writeln('vysl');
          //vypisBin(otocBin(vysl));     //addings is fcuked
        end;
...
var
  p, nahodne, pMensi: cisloT;
var
  bin, binP, binNahodne: cisloBin;
var
  d: vysledekDeleni;
var
